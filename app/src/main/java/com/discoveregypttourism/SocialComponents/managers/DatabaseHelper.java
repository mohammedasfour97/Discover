...
import com.discoveregypttourism.SocialComponents.managers.listeners.OnProfileCreatedListener;
...
//        Sets the maximum time to retry upload operations if a failure occurs.
    ...
    public void createOrUpdateProfile(final Profile profile, final OnProfileCreatedListener onProfileCreatedListener) {
        DatabaseReference databaseReference = ApplicationHelper.getDatabaseHelper(context).getDatabaseReference();
    ...
    public UploadTask uploadImage(Uri uri, String imageTitle) {
        ...
        StorageReference riversRef = storageRef.child("uploads/" + imageTitle +  "." + getFileExtension(uri));
        ...
        if (objectMap != null) {
            ...
            for (String key : objectMap.keySet()) {
                ...
                if (obj instanceof Map) {
                    ...
                    if (!hasComplain && new TinyDB(context).getListString("interests").contains((String) mapObj.get("title"))) {
                        Post post = new Post();
                        post.setId(key);
                        ...
                        post.setVideoPath((String) mapObj.get("videoPath"));
