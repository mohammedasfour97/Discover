...
package com.discoveregypttourism.SocialComponents.controllers;
...
import com.discoveregypttourism.SocialComponents.fragments.BaseFragment;
...
public class LikeController {
    ...
    private void addLike(long prevValue) {
        ...
        DatabaseHelper.getInstance(context).createOrUpdateLike(postId, postAuthorId);
    }
    ...
    private void removeLike(long prevValue) {
        ...
        isLiked = false;
        likeCounterTextView.setText(String.valueOf(prevValue - 1));
        DatabaseHelper.getInstance(context).removeLike(postId, postAuthorId);
    }
    ...
    public void handleLikeClickAction(final Context baseActivity, final Post post) {
            ...
            public void onDataChanged(boolean exist) {
                if (exist) {
                    if (hasInternetConnection(baseActivity)) {
                        ...
                        Toast.makeText(baseActivity, baseActivity.getResources().getString(R.string.internet_connection_failed), Toast.LENGTH_SHORT).show();
                    }
                } else {
                    Toast.makeText(baseActivity, baseActivity.getResources().getString(R.string.message_post_was_removed), Toast.LENGTH_SHORT).show();
                }
            }
        });
    }
    ...
    public void handleLikeClickAction(final BaseActivity baseActivity, final Post post) {
        PostManager.getInstance(baseActivity).isPostExistSingleValue(post.getId(), new OnObjectExistListener<Post>() {
            @Override
            public void onDataChanged(boolean exist) {
                if (exist) {
    ...
    private void showWarningMessage(BaseFragment baseActivity, int messageId) {
        if (baseActivity instanceof HomeFragment) {
            ((HomeFragment) baseActivity).showFloatButtonRelatedSnackBar(messageId);
        } else {
            baseActivity.showWarningDialog(messageId);
        }
    }
    ...
    private void doHandleLikeClickAction(Context baseActivity, Post post) {
        ProfileStatus profileStatus = ProfileManager.getInstance(baseActivity).checkProfile();
